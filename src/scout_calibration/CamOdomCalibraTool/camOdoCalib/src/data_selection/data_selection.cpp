                                 ...
                                 std::vector<data_selection::sync_data> &sync_result)
{
    ...
    for (int i = 3; unsigned(i) < camDatas.size() - 3; ++i)
    {
        ...
        {
            float alpha = (camDatas[i].start_t - odoDatas[odo_start_id].time) / (odoDatas[odo_start_id + 1].time - odoDatas[odo_start_id].time);
            alpha = fabs(alpha);
        ...
        {
            float alpha = (odoDatas[odo_start_id].time - camDatas[i].start_t) / (odoDatas[odo_start_id].time - odoDatas[odo_start_id - 1].time);
            alpha = fabs(alpha);
            v_left_start = (1 - alpha) * odoDatas[odo_start_id].v_left + alpha * odoDatas[odo_start_id - 1].v_left;
            v_right_start = (1 - alpha) * odoDatas[odo_start_id].v_right + alpha * odoDatas[odo_start_id - 1].v_right;
        }
        ...
        {
            float alpha = (camDatas[i].end_t - odoDatas[odo_end_id].time) / (odoDatas[odo_end_id + 1].time - odoDatas[odo_end_id].time);
            alpha = fabs(alpha);
        ...
        {
            float alpha = (odoDatas[odo_end_id].time - camDatas[i].end_t) / (odoDatas[odo_end_id].time - odoDatas[odo_end_id - 1].time);
            alpha = fabs(alpha);
            v_left_end = (1 - alpha) * odoDatas[odo_end_id].v_left + alpha * odoDatas[odo_end_id - 1].v_left;
            v_right_end = (1 - alpha) * odoDatas[odo_end_id].v_right + alpha * odoDatas[odo_end_id - 1].v_right;
        }
...
void data_selection::startPosAlign(OdomPtr &odoDatas, CamPtr &camDatas)
{
    ...
    if (t0_cam > t0_odo)
    {
        ...
        int odo_start = 0;
        while (fabs(odoDatas[++odo_start].time - t0_cam) < delta_t)
            delta_t = fabs(odoDatas[odo_start].time - t0_cam);
    ...
    else if (t0_odo > t0_cam)
    {
        ...
        int cam_start = 0;
        while (fabs(camDatas[++cam_start].start_t - t0_odo) < delta_t)
            delta_t = fabs(camDatas[cam_start].start_t - t0_odo);
