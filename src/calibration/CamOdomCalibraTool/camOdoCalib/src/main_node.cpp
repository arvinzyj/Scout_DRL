...
void wheel_callback(const OdomConstPtr &odo_msg)
{
    ...
    odo_tmp.time = time;
    odo_tmp.v_left = linear[0] / 0.1 - angular[2] * 0.49 / (2 * 0.1);  // linear velcity of x axis
    odo_tmp.v_right = linear[0] / 0.1 + angular[2] * 0.49 / (2 * 0.1); // angular velcity of z axis
...
void calc_process(const CameraPtr &cam)
{
    ...
    std::cout << std::endl
              << "images counts waiting for processing: " << std::endl;
    while (1)
    {
        cv::Mat image;
